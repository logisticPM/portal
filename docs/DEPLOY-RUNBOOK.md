...
> version. See the version-drift note in [`PROJECT-AUDIT.md`](./PROJECT-AUDIT.md) §5.
...
and is easy to run up during bulk extraction — see [`PROJECT-AUDIT.md`](./PROJECT-AUDIT.md) §3.
...
- Serverless infra ≈ free at demo scale; **real cost is Bedrock/BDA/Textract per
  page/token**. See [`PROJECT-AUDIT.md`](./PROJECT-AUDIT.md) §3 for real figures.
