...
(async () => {
  ...
  // Deliberately ABOVE the gold-coverage check below. `assert` throws on the first failure, and
  // that check is legitimately red from the moment the query set grew until the judging run
  // regenerates the gold — so while it sat first, every assertion in this block was unreachable
  // and had never run once. A guard that cannot execute until after the thing it guards is built
  // is not a guard. These are pure properties of EVAL_QUERIES and depend on no gold, so they
  // belong first.
  {
  ...
  // uncovered qid: this check is legitimately red from the moment the query set grows until the
  // judging run regenerates the gold, and every implementer in between runs this suite. "no gold
  // for query conceptual-007" tells them nothing about whether they broke something; a list plus
  // the reason tells them at a glance that it is the known gap.
  //
  // Still a hard failure — the invariant is real. Only the diagnostics and the position changed.
